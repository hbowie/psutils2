...
public class UserPrefs {
  ...
  public double getPrefAsDouble (String key, double defaultValue) {
    String pref = getPref (key);
    double prefDouble = 0.0;
    if (pref.length() == 0) {
      prefDouble = defaultValue;
    } else {
      try {
        prefDouble = Double.parseDouble(pref);
      } catch (NumberFormatException e) {
        prefDouble = defaultValue;
      }
    }
  ...
  public int getPrefAsComboBoxIndex (
    ...
    if (pref.length() == 0) {
      ...
      while (i < combo.getItems().size()) {
        Object comboObject = combo.getItems().get(i);
  ...
  public void setPref (String key, double data) {
    userPreferences.putDouble(key, data);
    prefsChanged();
  }
